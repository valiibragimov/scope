...
function extractAssetPathsFromHtml(html) {
    const assetMatches = Array.from(html.matchAll(/<(?:link|script)\b[^>]+(?:href|src)=["']([^"'#]+)["']/gi));
...
async function discoverInstallAssets() {
    ...
    try {
        ...
        discoveredAssets.push(...(manifest.icons || []).map((icon) => icon.src || ""), ...(manifest.shortcuts || []).flatMap((shortcut) => [
            shortcut.url || "",
...
self.addEventListener("install", (event) => {
    event.waitUntil((async () => {
        ...
        const urls = installAssets.map((asset) => new URL(asset, self.registration.scope).toString());
